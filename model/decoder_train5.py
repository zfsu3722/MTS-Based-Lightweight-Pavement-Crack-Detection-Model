...
for layer_name, layer_module in encoder.named_modules():
    if layer_name == ENCODER_ACTIVATION_LAYER:
        ...
        print(f"Forward hooks have been registered in the encoder layer： {ENCODER_ACTIVATION_LAYER}")
        target_layer_found = True
        break
if not target_layer_found:
     print(f"Warning: No encoder layer found for activating the hook '{ENCODER_ACTIVATION_LAYER}'。")
...
for epoch in range(1, num_epochs + 1):
    ...
    for i, label_tensor in enumerate(train_target):
        ...
        with torch.no_grad(): #
            for j in range(group_start_index, min(group_start_index + group_size, len(train_source_tensor))):
                 ...
                 if ENCODER_ACTIVATION_LAYER not in activation:
                      print(f"Error (Train): Activation layer '{ENCODER_ACTIVATION_LAYER}' Not in the sample {i}, index {j} turn up。")
                      group_activations = []
                      break
        ...
        try:
            ...
            print(f"Error (Train): Sample {i} an error occurred during splicing activation.: {e}")
            continue
        optimizer.zero_grad()
        ...
        try:
             ...
             print(f"Error (Training): Sample {i} An error occurred while calculating the loss: {e}")
             print(f"  Predicted shape: {train_pred_image.shape}, Label shape: {label.shape}")
             continue
        tloss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i, test_label_tensor in enumerate(test_target):
            ...
            for j in range(group_start_index, min(group_start_index + group_size, len(test_source_tensor))):
                ...
                if ENCODER_ACTIVATION_LAYER not in activation:
                    print(f"Error (valid): Activation layer '{ENCODER_ACTIVATION_LAYER}' Not in the sample {i}, index {j} find。")
                    test_group_activations = []
                    break
            ...
            try:
                ...
                test_tensor_out = torch.cat(test_group_activations, dim=1) #
                 ...
                 print(f"Error (validation): Sample {i} An error occurred during splicing activation.: {e}")
                 continue
            ...
            try:
                 ...
                 print(f"Error (validation): Sample {i} An error occurred while calculating the loss.: {e}")
                 print(f"  Predicted shape: {output.shape}, lable shape: {test_label.shape}")
                 continue
                 ...
                 try:
                     ...
                     if final_results:
                         f1 = final_results[0][3]
                          ...
                          print("Warning: The PRF metric calculation returned an empty result.")
                          f1 = 0.0
                 except Exception as e:
                      print(f"Error: An error occurred while calculating the metrics.: {e}")
                      f1 = 0.0
                 if f1 > best_f1:
                     best_f1 = f1
                     ...
                     print(f"*** New best F1 score： {best_f1:.6f} in epoch {best_epoch}。 ***")
                     ...
                     for idx, (pred_b, test_l, test_ln) in enumerate(zip(best_pred_list, best_test_label_list, test_label_names)):
                         if idx < len(test_label_names):
                             ...
                             cv2.imwrite(output_filename_gt, test_label_array) #
                              ...
                              print(f"Warning (Save best image): Index {idx} Outside the range of test_label_names.")
            else:
                print("Unknown mode of metrics.")
        else:
             print("Warning: The validation prediction/label list is empty; metrics cannot be calculated.。")
             f1 = 0.0
    epoch_checkpoint = {
